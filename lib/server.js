...
app.post('/api/upload_profile_picture', profilePictureUpload.single('profile_picture'), (req, res) => {
  ...
  connection.query('UPDATE users SET profile_picture = ? WHERE id = ?', [profilePictureUrl, userEmail], (error, results) => {
    if (error) {
...
app.get('/api/profile_picture/:userEmail', (req, res) => {
  const userEmail = req.params.userEmail; // Correct parameter name

  connection.query('SELECT profile_picture FROM users WHERE email = ?', [userEmail], (error, results) => {
    if (error) {
...
app.post('/api/register1', async (req, res) => {
  ...
  connection.query('SELECT * FROM users WHERE email = ?', [email], async (error, results) => {
    ...
    try {
      ...
      connection.query(insertQuery, [name, email, hashedPassword, role], (error, results) => {
        if (error) {
          console.error('Error inserting user:', error);
          return res.status(500).send('Internal Server Error');
        }
        ...
        res.status(201).json({ 
          ...
          user: { id: results.insertId, name, email, role } 
        });
      });
    } catch (hashError) {
